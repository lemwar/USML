    ...
    def crossing_over(self, cross_pop, is_multi=False):
        ...
        for i in range(0, len(cross_pop)-1, 2):
            if random.random() <= self.cross_prob:
                ...
                if is_multi:
                    k1 = random.randint(1, len(chromosome_1) - 2)
                    k2 = random.randint(k1 + 1, len(chromosome_1) - 1)
                    new_chromosome_1 = chromosome_1[0:k1] + chromosome_2[k1:k2] + chromosome_1[k2:self.n_bit]
                    new_population.append(new_chromosome_1)

                    new_chromosome_2 = chromosome_2[0:k1] + chromosome_1[k1:k2] + chromosome_2[k2:self.n_bit]
                    new_population.append(new_chromosome_2)
                else:
                    k1 = random.randint(1, len(chromosome_1) - 2)
                    new_chromosome_1 = chromosome_1[0:k1] + chromosome_2[k1:self.n_bit]
                    new_population.append(new_chromosome_1)

                    new_chromosome_2 = chromosome_2[0:k1] + chromosome_1[k1:self.n_bit]
                    new_population.append(new_chromosome_2)
